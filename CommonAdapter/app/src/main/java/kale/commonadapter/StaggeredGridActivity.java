...
import kale.commonadapter.item.StaggerredGredImageItem;
import kale.commonadapter.item.StaggerredGridButtonItem;
import kale.commonadapter.item.StaggerredGridTextItem;
...
//		        LinearLayoutManager.VERTICAL, false);
	...
	class RcvAdapter extends CommonRcvAdapter<DemoModel> {
		...
		protected AdapterItem<DemoModel> initItemView(Object type) {
			...
			switch ((String) type) {
				case "text":
					return new StaggerredGridTextItem();
				case "button":
					return new StaggerredGridButtonItem();
				case "image":
					return new StaggerredGredImageItem();
				default:
					Log.d(TAG, "default-----------");
					return new TextItem();
			}
